...
impl Commit {
  pub fn from(raw_commit: &str, separator: &str, tags_re: &Regex) -> Self {
    ...
    let possible_tag = match tags_re.captures(raw_subject) {
      Some(tag) => tag.iter()
        .filter_map(|possible_tag| if possible_tag.is_some() { possible_tag } else { None })
        .last(),
      None => None
    };
...
mod test {
  ...
  fn creates_with_matching_tag_from_string() {
    ...
    let commit_feat = Commit::from(
      "feat: Sample message here|author|hash",
      "|",
      &tags_re);

    let commit_story = Commit::from(
      "US123 Sample message here|author|hash",
      "|",
      &tags_re);

    let commit_chore = Commit::from(
      "chore: Sample message here|author|hash",
      "|",
      &tags_re);
